...
class sieve():
    ...
    def __init__(self, n=6, deg=None, a=0, b=1, btype='chebyshev', gridtype='c', knots=None):
        '''Initializer'''
        
        self.n=np.atleast_1d(n)
        if deg is None: deg=self.n-1
        self.k=len(self.n);
    ...
    def fit(self, y, x=None):
        ...
        if x is not None:
            ...
            B = sieve.basis(z, self.deg, self.btype)
            return np.linalg.inv(B.T@B)@B.T@y    
        else:
            return self.pB@y

    def eval(self, x, c):
        b=sieve.basis(self.x2z(x), self.deg,  self.btype)
        return b@c
    ...
    def basis(z, deg, btype, knots=None):
        ...
        k=z.shape[1]
        for j in range(k): 
            Bj=sieve.basis_j(z[:,j], deg[j], btype[j], None)
            B.append(Bj)
    ...
    def basis_j(z, deg, btype='chebyshev', knots=None):
        ...
        if btype=='algpol':      # Simple powers of x: [x^0, x^1, x^2, ... x^deg]
            ...
            raise RuntimeError('b-spline not implmeneted')
        else: 
